...
type {{$name}} struct {
    ...
    {{range .Fields -}}
		{{$fieldName := .Name}}
    	{{$fieldName}} {{.Type}} `{{with .GormTag}}gorm:"{{with .Tag}}{{.}}{{else}}{{with .ColumnName}}column:{{.}};{{end}}{{with .Index}}index;{{end}}{{with .Size}}size:{{.}};{{end}}{{end}}"{{end}} {{- with .JSONTag}} json:"{{if .Tag}}{{.Tag}}{{else}}{{lowerUnderline $fieldName}}{{if .OmitEmpty}},omitempty{{end}}{{end}}"{{end}}`{{with .Comment}}// {{.}}{{end}}
    {{- end -}}
